...
function getGeneratedAgentsMdPath(result) {
  const parsed = JSON.parse(result.output);
  return parsed.agents_md_path || parsed.claude_md_path;
}
...
describe('generate-claude-md', () => {
  ...
  test('creates CLAUDE.md with workflow enforcement section', () => {
    ...
    const agentsMdPath = getGeneratedAgentsMdPath(result);
  ...
  test('adds workflow enforcement section when updating an existing CLAUDE.md', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, 'AGENTS.md'), '## Local Notes\n\nKeep this intro.\n');
...
describe('new-project workflow includes canonical AGENTS.md generation', () => {
  ...
  test('new-project workflow generates instruction file before final commit', () => {
    ...
    assert.ok(content.includes('generate-agents-md'));
    // Codex-first fork: workflow now treats AGENTS.md as the canonical instruction file.
    assert.ok(
  ...
  test('new-project artifacts reference instruction file variable', () => {
    ...
    assert.ok(commandsContent.includes('`AGENTS.md`'));
  });
});
...
describe('generate-claude-md skills section', () => {
  ...
  test('includes skills fallback when no skills directories exist', () => {
    ...
    const content = fs.readFileSync(getGeneratedAgentsMdPath(result), 'utf-8');
  ...
  test('discovers skills from .claude/skills/ directory', () => {
    const skillDir = path.join(tmpDir, '.codex', 'skills', 'api-payments');
  ...
  test('skips gsd- prefixed skill directories', () => {
    const gsdSkillDir = path.join(tmpDir, '.codex', 'skills', 'gsd-plan-phase');
    const userSkillDir = path.join(tmpDir, '.codex', 'skills', 'my-feature');
  ...
  test('handles multi-line description in frontmatter', () => {
    const skillDir = path.join(tmpDir, '.codex', 'skills', 'complex-skill');
    fs.mkdirSync(skillDir, { recursive: true });
    fs.writeFileSync(
      path.join(skillDir, 'SKILL.md'),
  ...
  test('deduplicates skills found in multiple directories', () => {
    // Same skill in both .codex/skills/ and .agents/skills/
    const dir1 = path.join(tmpDir, '.codex', 'skills', 'shared-skill');
  ...
  test('updates existing skills section on regeneration', () => {
    ...
    const firstResult = runGsdTools('generate-claude-md', tmpDir);
    assert.ok(firstResult.success, `Command failed: ${firstResult.error}`);
    let content = fs.readFileSync(getGeneratedAgentsMdPath(firstResult), 'utf-8');
    assert.ok(content.includes('No project skills found'));

    // Add a skill and regenerate
    const skillDir = path.join(tmpDir, '.codex', 'skills', 'new-skill');
    fs.mkdirSync(skillDir, { recursive: true });
    fs.writeFileSync(
      path.join(skillDir, 'SKILL.md'),
    ...
    content = fs.readFileSync(getGeneratedAgentsMdPath(result), 'utf-8');
    assert.ok(!content.includes('No project skills found'));
  ...
  test('skills section appears between architecture and workflow', () => {
    const skillDir = path.join(tmpDir, '.codex', 'skills', 'ordering-test');
    fs.mkdirSync(skillDir, { recursive: true });
    fs.writeFileSync(
      path.join(skillDir, 'SKILL.md'),

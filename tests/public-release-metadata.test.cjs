...
describe('public release metadata', () => {
  ...
  test('current patch release notes cover public trust and discovery updates', () => {
    const changelog = fs.readFileSync(path.join(ROOT, 'CHANGELOG.md'), 'utf8');
    const releaseStart = changelog.indexOf('## [1.37.3]');
    const nextReleaseStart = changelog.indexOf('## [1.37.1]');

    assert.ok(releaseStart > -1, 'CHANGELOG should include a 1.37.3 release section');
    assert.ok(nextReleaseStart > releaseStart, '1.37.3 section should precede 1.37.1');

    const releaseNotes = changelog.slice(releaseStart, nextReleaseStart);

    assert.match(releaseNotes, /npm discovery keywords/);
    assert.match(releaseNotes, /README quick value scan/);
    assert.match(releaseNotes, /60-second workflow/);
    assert.match(releaseNotes, /SUPPORT\.md/);
    assert.match(releaseNotes, /GitHub Discussions/);
    assert.match(releaseNotes, /Public support routing/);
  });

...
describe('claude_md_path config key', () => {
  ...
  test('config template no longer uses claude_md_path as the default key', () => {
    const templatePath = path.join(__dirname, '..', 'get-shit-done', 'templates', 'config.json');
    const template = JSON.parse(fs.readFileSync(templatePath, 'utf-8'));
    assert.ok(!('claude_md_path' in template));
    assert.strictEqual(template.agents_md_path, './AGENTS.md');
  });
  ...
  test('buildNewProjectConfig migrates defaults to agents_md_path', () => {
    ...
    assert.ok(!('claude_md_path' in config));
    assert.strictEqual(config.agents_md_path, './AGENTS.md');
  });
});

describe('agents_md_path config key', () => {
  let tmpDir;
  ...
  test('agents_md_path is in VALID_CONFIG_KEYS', () => {
    const { VALID_CONFIG_KEYS } = require('../get-shit-done/bin/lib/config.cjs');
    assert.ok(VALID_CONFIG_KEYS.has('agents_md_path'));
  });

  test('config template includes agents_md_path', () => {
    const templatePath = path.join(__dirname, '..', 'get-shit-done', 'templates', 'config.json');
    const template = JSON.parse(fs.readFileSync(templatePath, 'utf-8'));
    assert.strictEqual(template.agents_md_path, './AGENTS.md');
  });

  test('config-get agents_md_path returns default value when not set', () => {
    const configPath = path.join(tmpDir, '.planning', 'config.json');
    fs.writeFileSync(configPath, JSON.stringify({ mode: 'interactive' }), 'utf-8');

    const result = runGsdTools('config-get agents_md_path --default ./AGENTS.md', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Expected success but got error: ${result.error}`);
    assert.strictEqual(JSON.parse(result.output), './AGENTS.md');
  });

  test('config-set agents_md_path works', () => {
    const configPath = path.join(tmpDir, '.planning', 'config.json');
    fs.writeFileSync(configPath, JSON.stringify({ mode: 'interactive' }), 'utf-8');

    const setResult = runGsdTools('config-set agents_md_path .codex/AGENTS.md', tmpDir, { HOME: tmpDir });
    assert.ok(setResult.success, `Expected success but got error: ${setResult.error}`);

    const getResult = runGsdTools('config-get agents_md_path', tmpDir, { HOME: tmpDir });
    assert.ok(getResult.success, `Expected success but got error: ${getResult.error}`);
    assert.strictEqual(JSON.parse(getResult.output), '.codex/AGENTS.md');
  });

  test('buildNewProjectConfig includes agents_md_path default', () => {
    const result = runGsdTools('config-new-project', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Expected success but got error: ${result.error}`);
...
describe('cmdGenerateClaudeMd reads claude_md_path from config', () => {
  ...
  test('defaults to ./AGENTS.md when config has no explicit instruction path', () => {
    ...
    assert.strictEqual(parsed.agents_md_path, path.join(realTmpDir, 'AGENTS.md'));
  });
});

describe('cmdGenerateAgentsProfile reads agents_md_path from config', () => {
  let tmpDir;
  ...
  test('uses agents_md_path from config when no --output or --global', () => {
    const configPath = path.join(tmpDir, '.planning', 'config.json');
    const customPath = '.codex/AGENTS.md';
    fs.writeFileSync(configPath, JSON.stringify({ agents_md_path: customPath }), 'utf-8');
    ...
    const result = runGsdTools(
      ['generate-agents-profile', '--analysis', analysisPath],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Expected success but got error: ${result.error}`);
    ...
    const realTmpDir = fs.realpathSync(tmpDir);
    const expectedPath = path.join(realTmpDir, customPath);
    assert.strictEqual(parsed.agents_md_path, expectedPath);
    assert.ok(fs.existsSync(expectedPath), `Expected file at ${expectedPath}`);
  });

  test('legacy generate-claude-profile alias reads agents_md_path from config', () => {
    const configPath = path.join(tmpDir, '.planning', 'config.json');
    const customPath = '.codex/AGENTS.md';
    fs.writeFileSync(configPath, JSON.stringify({ agents_md_path: customPath }), 'utf-8');
    ...
    const parsed = JSON.parse(result.output);
    const realTmpDir = fs.realpathSync(tmpDir);
    assert.strictEqual(parsed.claude_md_path, path.join(realTmpDir, customPath));
  });
});

describe('cmdGenerateAgentsMd reads agents_md_path from config', () => {
  let tmpDir;
  ...
  test('uses agents_md_path from config when no --output', () => {
    const configPath = path.join(tmpDir, '.planning', 'config.json');
    const customPath = '.codex/AGENTS.md';
    fs.writeFileSync(configPath, JSON.stringify({ agents_md_path: customPath }), 'utf-8');
    ...
    const result = runGsdTools('generate-agents-md', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Expected success but got error: ${result.error}`);
  ...
  test('legacy generate-claude-md alias uses agents_md_path from config', () => {
    const configPath = path.join(tmpDir, '.planning', 'config.json');
    const customPath = '.codex/AGENTS.md';
    fs.writeFileSync(configPath, JSON.stringify({ agents_md_path: customPath }), 'utf-8');

...
Use the [Safe Trial Outcome Template](SAFE-TRIAL-OUTCOME.md) when you want to record the decision. Use [Safe Trial Troubleshooting](SAFE-TRIAL-TROUBLESHOOTING.md) if install, command discovery, or changed-file checks block the decision. Use the [Safe Trial Discussion Starter](SAFE-TRIAL-DISCUSSION.md) when you want to ask in GitHub Discussions with enough evidence to act on.

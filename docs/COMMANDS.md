...
- **Canonical in this fork:** `$gsd-command-name [args]`
- **Claude Code / Gemini / Copilot compatibility syntax:** `/gsd-command-name [args]`
- **OpenCode / Kilo compatibility syntax:** `/gsd-command-name [args]`

Codex command names are the release-facing default in this repository. Slash-command forms remain valid runtime-specific aliases, but public examples should be read as compatibility syntax unless noted otherwise.

## Why This Fork Uses Codex Syntax First

This fork is published for Codex-first usage. New users should see `$gsd-*` commands, `AGENTS.md`, and `.codex/` first instead of learning Claude-era names and then translating them back to Codex.

Compatibility still exists for users migrating older installs or working across multiple runtimes, but compatibility is intentionally secondary in these docs.

## Current Stability

- Stable for the canonical `$gsd-*` command surface in this fork
- Stable for legacy slash-command aliases where runtime integrations still expect them
- Public command docs prefer Codex-first examples while keeping factual compatibility references
...
| Flag | Description |
|------|-------------|
| `--all` | Skip area selection 鈥?discuss all gray areas interactively (no auto-advance) |
...
**Behavior:**
- No project 鈫?suggests `/gsd-new-project`
- Phase needs discussion 鈫?runs `/gsd-discuss-phase`
- Phase needs planning 鈫?runs `/gsd-plan-phase`
- Phase needs execution 鈫?runs `/gsd-execute-phase`
- Phase needs verification 鈫?runs `/gsd-verify-work`
- All phases complete 鈫?suggests `/gsd-complete-milestone`
...
```bash
/gsd-add-phase                      # Interactive 鈥?describe the phase
```
...
```bash
/gsd-insert-phase 3                 # Insert between phase 3 and 4 鈫?creates 3.1
```
...
```bash
/gsd-remove-phase 7                 # Remove phase 7, renumber 8鈫?, 9鈫?, etc.
```

### `/gsd-list-phase-assumptions`

Preview the agent's intended approach before planning.
...
**Detection methods:**
- File overlap 鈥?phases touching the same files/domains must be ordered
- Semantic dependencies 鈥?a phase that consumes an API or schema built by another phase
- Data flow 鈥?a phase that reads output produced by another phase
...
Deep ecosystem research only (standalone 鈥?usually use `/gsd-plan-phase` instead).
...
Socratic ideation session 鈥?guide an idea through probing questions, optionally spawn research, then route output to the right GSD artifact (notes, todos, seeds, research questions, requirements, or a new phase).
...
Safe git revert 鈥?roll back GSD phase or plan commits using the phase manifest with dependency checks and a confirmation gate.
...
**Process:** Detects conflicts 鈫?prompts for resolution 鈫?writes as GSD PLAN.md 鈫?validates via `gsd-plan-checker`
...
Reverse migration from GSD-2 format (`.gsd/` with Milestone鈫扴lice鈫扵ask hierarchy) back to v1 `.planning/` format.
...
**Flattening:** Milestone鈫扴lice hierarchy is flattened to sequential phase numbers (M001/S01鈫抪hase 01, M001/S02鈫抪hase 02, M002/S01鈫抪hase 03, etc.).
...
Zero-friction idea capture 鈥?append, list, or promote notes to todos.
...
| Flag | Description |
|------|-------------|
| `--diagnose` | Diagnosis-only mode 鈥?investigate without attempting fixes |

**Subcommands:**
- `/gsd-debug list` 鈥?List all active debug sessions with status, hypothesis, and next action
- `/gsd-debug status <slug>` 鈥?Print full summary of a session (Evidence count, Eliminated count, Resolution, TDD checkpoint) without spawning an agent
- `/gsd-debug continue <slug>` 鈥?Resume a specific session by slug (surfaces Current Focus then spawns continuation agent)
- `/gsd-debug [--diagnose] <description>` 鈥?Start new debug session (existing behavior; `--diagnose` stops at root cause without applying fix)

**TDD mode:** When `tdd_mode: true` in `.planning/config.json`, debug sessions require a failing test to be written and verified before any fix is applied (red 鈫?green 鈫?done).
...
**Generated artifacts:**
- `USER-PROFILE.md` 鈥?Full behavioral profile
- `$gsd-dev-preferences` command 鈥?Load preferences in any Codex session
- `AGENTS.md` profile section 鈥?Auto-discovered by Codex
...
Run 2鈥? focused feasibility experiments before committing to an implementation approach. Each experiment uses Given/When/Then framing, produces executable code, and returns a VALIDATED / INVALIDATED / PARTIAL verdict.
...
**Produces:** `.codex/skills/spike-findings-[project]/` skill file
...
Explore design directions through throwaway HTML mockups before committing to implementation. Produces 2鈥? variants per design question for direct browser comparison.

| Argument | Required | Description |
|----------|----------|-------------|
| `idea` | No | The UI design question or direction to explore |
| `--quick` | No | Skip mood intake; use `idea` text directly |
| `--text` | No | Text-mode fallback 鈥?replace interactive prompts with numbered lists (for non-Claude runtimes) |

**Produces:** `.planning/sketches/NNN-descriptive-name/index.html` (2鈥? interactive variants), `README.md`, shared `themes/default.css`; `.planning/sketches/MANIFEST.md`
...
**Produces:** `.codex/skills/sketch-findings-[project]/` skill file
...
```bash
/gsd-forensics                              # Interactive 鈥?prompted for problem
...
Rapid single-focus codebase assessment 鈥?lightweight alternative to `/gsd-map-codebase` that spawns one mapper agent instead of four parallel ones.
...
| `--depth=quick\|standard\|deep` | No | Review depth level (overrides `workflow.code_review_depth` config). `quick`: pattern-matching only (~2 min). `standard`: per-file analysis with language-specific checks (~5鈥?5 min, default). `deep`: cross-file analysis including import graphs and call chains (~15鈥?0 min) |
...
Autonomous audit-to-fix pipeline 鈥?runs an audit, classifies findings, fixes auto-fixable issues with test verification, and commits each fix atomically.
...
Execute a trivial task inline 鈥?no subagents, no planning overhead. For typo fixes, config changes, small refactors, forgotten commits.

| Argument | Required | Description |
|----------|----------|-------------|
| `task description` | No | What to do (prompted if omitted) |

**Not a replacement for `/gsd-quick`** 鈥?use `/gsd-quick` for anything needing research, multi-step planning, or verification.
...
**Produces:** `{phase}-REVIEWS.md` 鈥?consumable by `/gsd-plan-phase --reviews`
...
1. SECURITY.md exists 鈥?audit and verify existing mitigations
2. No SECURITY.md but PLAN.md has threat model 鈥?generate from artifacts
3. Phase not executed 鈥?exits with guidance
...
Each doc writer explores the codebase directly 鈥?no hallucinated paths or stale signatures. Doc verifier checks claims against the live filesystem.
...
Capture a forward-looking idea with trigger conditions 鈥?surfaces automatically at the right milestone.
...
| Argument | Required | Description |
|----------|----------|-------------|
| (none) | 鈥?| List all threads |
| `name` | 鈥?| Resume existing thread by name |
| `description` | 鈥?| Create new thread |
...
| Flag | Description |
|------|-------------|
| `--verify` | Dry-run mode 鈥?show proposed changes without writing |

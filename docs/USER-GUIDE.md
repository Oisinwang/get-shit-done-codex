...
requirement before any code is written. This ensures that when the executor
...
AI-generated frontends are visually inconsistent not because the model is bad at UI but because no design contract existed before execution. Five components built without a shared spacing scale, color contract, or copywriting standard produce five slightly different visual decisions.
...
$gsd-sketch --text "onboarding flow"    # For text-only or compatibility runtimes
```
...
| `$gsd-list-phase-assumptions [N]` | Preview the planned approach | Before planning, to validate direction |
...
- **inherit** -- All agents use the current session model. Best when switching models dynamically (e.g. OpenCode or Kilo `/model`), or when using Claude Code with non-Anthropic providers (OpenRouter, local models) to avoid unexpected API costs. For Codex and compatibility runtimes (OpenCode, Gemini CLI, Kilo), the installer sets `resolve_model_ids: "omit"` automatically -- see [Compatibility Runtimes](#using-compatibility-runtimes-opencode-gemini-cli-kilo).
...
```bash
# Start a Codex session in this repo, then run:
$gsd-new-project            # Answer questions, configure, approve roadmap
# Clear context between major steps if your runtime supports it.
...
For automation and copy-paste from docs, prefer **`gsd-sdk query`** with a registered subcommand (see [CLI-TOOLS.md](CLI-TOOLS.md) and [QUERY-HANDLERS.md](../sdk/src/query/QUERY-HANDLERS.md)). The legacy **`node $HOME/.codex/get-shit-done/bin/gsd-tools.cjs`** CLI remains supported for dual-mode operation.
...
```bash
node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" state validate          # Detect drift between STATE.md and filesystem
node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" state sync --verify     # Preview what sync would change
node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" state sync              # Reconstruct STATE.md from disk
```
...
Run `$gsd-discuss-phase [N]` before planning. Most plan quality issues come from the agent making assumptions that `CONTEXT.md` would have prevented. You can also run `$gsd-list-phase-assumptions [N]` to see what the agent intends to do before committing to a plan.
...
To enable this: run `$gsd-profile-user` to generate your profile. The profile is stored at `~/.codex/get-shit-done/USER-PROFILE.md` and is read automatically on every `$gsd-discuss-phase` invocation. No other configuration is required.
...
Switch to budget profile: `$gsd-set-profile budget`. Disable research and plan-check agents via `$gsd-settings` if the domain is familiar to you or already well documented.

### Using Compatibility Runtimes (OpenCode, Gemini CLI, Kilo)

If you installed GSD for a compatibility runtime, the installer already configured model resolution so all agents use the runtime's default model. No manual setup is needed. Specifically, the installer sets `resolve_model_ids: "omit"` in your config, which tells GSD to skip Anthropic model ID resolution and let the runtime choose its own default model.

To assign different models to different agents on a compatibility runtime, add `model_overrides` to `.planning/config.json` with fully-qualified model IDs that your runtime recognizes:
...
The installer auto-configures `resolve_model_ids: "omit"` for Codex, Gemini CLI, OpenCode, and Kilo. If you're manually setting up a compatibility runtime, add it to `.planning/config.json` yourself.
...
npx @oisinwang/get-shit-done-codex --cline --global
...
```bash
npx @oisinwang/get-shit-done-codex --qwen --global
```
...
If `npx @oisinwang/get-shit-done-codex@latest` fails due to npm outages or network restrictions, see [docs/manual-update.md](manual-update.md) for a step-by-step manual update procedure that works without npm access.

...
Runs inline 閳?no Task, no AskUserQuestion, no Bash.
</purpose>
...
- **Project scope**: `.planning/notes/{YYYY-MM-DD}-{slug}.md` 閳?used when `.planning/` exists in cwd
- **Global scope**: `~/.codex/notes/{YYYY-MM-DD}-{slug}.md` 閳?fallback when no `.planning/`, or when `--global` flag is present
...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/gsd-note list of groceries` saves a note with text "list of groceries". Same for `promote` 閳?only a subcommand when followed by exactly one number.
</step>

<step name="append">
**Subcommand: append 閳?create a timestamped note file.**

1. Determine scope (project or global) per storage format above
2. Ensure the notes directory exists (`.planning/notes/` or `~/.codex/notes/`)
...
**Constraints:**
- **Never modify the note text** 閳?capture verbatim, including typos
- **Never ask questions** 閳?just write and confirm
- **Timestamp format**: Use local time, `YYYY-MM-DD HH:mm` (24-hour, no seconds)
</step>

<step name="list">
**Subcommand: list 閳?show notes from both scopes.**

1. Glob `.planning/notes/*.md` (if directory exists) 閳?project notes
2. Glob `~/.codex/notes/*.md` (if directory exists) 閳?global notes
...
<step name="promote">
**Subcommand: promote 閳?convert a note into a todo.**
...
4. **Requires `.planning/` directory** 閳?if it doesn't exist, warn: "Todos require a GSD project. Run `/gsd-new-project` to initialize one."
...
2. **No `.planning/`**: Falls back to global `~/.codex/notes/` 閳?works in any directory
...
6. **`--global` position**: Stripped from anywhere 閳?`--global my idea` and `my idea --global` both save "my idea" globally
...
- [ ] Append: No questions asked 閳?instant capture

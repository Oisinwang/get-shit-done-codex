...
### Added
- **Localized docs index runtime compatibility table links** - Adds `docs/FEATURES.md#36-multi-runtime-support`, `docs/ARCHITECTURE.md#runtime-abstraction`, `docs/USER-GUIDE.md#using-compatibility-runtimes-opencode-gemini-cli-kilo`, `docs/CONFIGURATION.md#non-claude-runtimes-codex-opencode-gemini-cli-kilo`, and `docs/manual-update.md#runtime-flags` table links to localized docs indexes so non-English readers can find supported runtimes, install formats, runtime-specific configuration, model inheritance, and update flags from localized entry points.
- **Localized docs index installer script table links** - Adds `bin/install.js`, `scripts/build-hooks.js`, `scripts/safe-trial-demo.cjs`, `scripts/run-tests.cjs`, and `scripts/validate-hotfix.cjs` table links to localized docs indexes so non-English readers can find the Codex-first installer entrypoint, hook build step, safe trial preview, Node test orchestration, and hotfix release validation scripts from localized entry points.
- **Localized docs index repository automation table links** - Adds `.github/workflows/release.yml`, `.github/workflows/pr-gate.yml`, `.github/workflows/require-issue-link.yml`, `.github/workflows/branch-naming.yml`, `.github/workflows/branch-cleanup.yml`, `.github/workflows/close-draft-prs.yml`, `.github/workflows/auto-label-issues.yml`, `.github/workflows/auto-branch.yml`, `.github/workflows/security-scan.yml`, `.github/workflows/stale.yml`, `.github/dependabot.yml`, `.github/labels.json`, `.github/CODEOWNERS`, and `.github/FUNDING.yml` table links to localized docs indexes so non-English contributors can find release automation, PR gates, issue-link enforcement, branch hygiene, draft rejection, triage labels, label-created branches, security scans, stale cleanup, dependency update cadence, label contracts, maintainer review ownership, and funding metadata from localized entry points.
- **Localized docs index pull request template table links** - Adds `.github/pull_request_template.md`, `.github/PULL_REQUEST_TEMPLATE/fix.md`, `.github/PULL_REQUEST_TEMPLATE/enhancement.md`, and `.github/PULL_REQUEST_TEMPLATE/feature.md` table links to localized docs indexes so non-English contributors can find the typed template chooser, `codex/bootstrap` branch target, Codex-first contract, approved issue requirements, no-draft rule, test evidence, scope confirmation, platform/runtime checks, and changelog/docs expectations from localized entry points.
...
- **Localized docs index promotion table links** - Adds `docs/PROMOTION.md` table links to localized docs indexes so non-English maintainers can find launch copy, social preview setup, and public positioning snippets from localized entry points.
- **Localized docs index maintainer checklist table links** - Adds `docs/MAINTAINER-CHECKLIST.md` table links to localized docs indexes so non-English maintainers can find resolved good-first issue closure guidance with verification, CI, label cleanup, and public notes from localized entry points.
...
- **Safe trial discussion starter** - Adds `docs/SAFE-TRIAL-DISCUSSION.md` with a copy-pastable GitHub Discussions post, command evidence, changed-path prompts, and a privacy check for safe local trial users.
...
- **Source docs versus published package docs troubleshooting** - Explains how to compare source docs on `codex/bootstrap` with published npm package docs and include `npm view @oisinwang/get-shit-done-codex version` evidence before reporting stale documentation.

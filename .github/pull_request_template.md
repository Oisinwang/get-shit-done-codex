...
This fork's public branch is `codex/bootstrap`, and public behavior is Codex-first:
`AGENTS.md`, `.codex/`, `agents_md_path`, and `generate-agents-*` are the primary contract.
If your PR touches legacy `.claude/`, `claude_*`, or `generate-claude-*` paths, explain why the
compatibility layer still needs that change.
...
- If you are not sure -> open a [Discussion](https://github.com/Oisinwang/get-shit-done-codex/discussions) first
...
> **No draft PRs.** Draft PRs are automatically closed. Only open a PR when your code is complete, tests pass, targets `codex/bootstrap`, and uses the correct template. See [CONTRIBUTING.md](../CONTRIBUTING.md).

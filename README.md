...
From a source checkout, `npm run demo:safe-trial` prints the same safe trial flow without installing or editing files. You can also read the [Safe Trial Transcript](docs/SAFE-TRIAL-TRANSCRIPT.md) first, then use the [Safe Trial Outcome Template](docs/SAFE-TRIAL-OUTCOME.md) to decide what to keep, the [Safe Trial Discussion Starter](docs/SAFE-TRIAL-DISCUSSION.md) to ask for workflow advice, or [Safe Trial Troubleshooting](docs/SAFE-TRIAL-TROUBLESHOOTING.md) if the first run gets stuck.
...
| Summarize a session for handoff | `$gsd-session-report` | `.planning/reports/SESSION_REPORT.md` for review or sharing |
| Capture a project health snapshot | `$gsd-stats` | Phases, plans, requirements, git metrics, and timeline summary |
| Archive completed milestone phases | `$gsd-cleanup` | `.planning/milestones/v{version}-phases/` archives after confirmation |
